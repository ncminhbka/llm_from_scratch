...
def estimate_loss(model: GPT, ds: ByteDataset, args) -> dict:
    ...
    model.train()
    return out #dict với keys là 'train' và 'val', values là loss trung bình tương ứng
...
def main():
    ...
    p.add_argument('--steps', type=int, default=2000) # số bước huấn luyện (epochs)
    p.add_argument('--lr', type=float, default=3e-4)
    p.add_argument('--weight_decay', type=float, default=0.1)
    p.add_argument('--grad_clip', type=float, default=1.0)
    p.add_argument('--eval_interval', type=int, default=200)
    p.add_argument('--eval_iters', type=int, default=50) # số lần lặp để đánh giá (không đánh giá toàn bộ tập dữ liệu)
    p.add_argument('--sample_every', type=int, default=200) # sau một số bước train lại sinh thử xem kết quả
    ...
    # optionally compile the model to speed up training
