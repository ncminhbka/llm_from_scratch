...
def top_k_top_p_filtering(logits: torch.Tensor, top_k: int | None = None, top_p: float | None = None):
    ...
    if top_k is not None and top_k < V:
        topk_vals, _ = torch.topk(filtered, top_k, dim=-1) # k giá trị lớn nhất
        kth = topk_vals[:, -1].unsqueeze(-1) 
        filtered[filtered < kth] = float('-inf') #sau kth tất cả về -inf
    ...
    return filtered
'''
Top-k = chỉ cho phép model chọn k token có xác suất cao nhất tại mỗi bước.
Top-p = chọn những token có tổng xác suất ≥ p từ cao xuống thấp.
có thể dùng 1 trong 2 hoặc cả 2
'''